/**
 * @file
 * Routines to read camera configuration (.cfg) files and populate
 * a PCI DV dependent structure for use by pdv_initcam.
 *
...
/* utility routine for embedded (nofs) config file arrays -- copies and changes '\"' to '"'*/
    char *
get_next_string(int nofs_cfg, FILE *cfg_fp, char *ss, char *ds)
{
#ifdef NO_FS
    char *dp = ds;
    char *sp ;
    if (nofs_cfg)
    {
        ...
        *dp = '\0';
        ...
        return fgets(ds, MAXINIT, cfg_fp);
}
...
char *dmy_cfg[] = {""};
    ...
    void
pdv_dep_set_default(PdvDependent * dd_p)

{
    ...
    int
readcfg(const char *cfgfile, Dependent * dd_p, Edtinfo * ei_p, int nofs_cfg)
{
    ...
    edt_msg(DEBUG2, "Reading/processing config file %s:\n\n", cfgfile);
    ...
    while (get_next_string(nofs_cfg, cfg_fp, *vx_p, s))
    {
        ...
        if ((ret = check_other_param(s, dd_p, ei_p, cfgfile)) < 0)
            return -1;
    ...
    /* deal with no taps/depth fields */
...
/**
 * Reads a configuration file and fills in the dependent and edtinfo
 * structures based on the information in the file.  These structures
 * can then be passed in to #pdv_initcam to initialize the board and
 * camera.
 *
 ...
 */
    int
pdv_readcfg(const char *cfgfile, Dependent * dd_p, Edtinfo *ei_p)
{
    return readcfg(cfgfile, dd_p, ei_p, 0);
}

    int
pdv_readcfg_emb(char *cfgfile, Dependent * dd_p, Edtinfo *ei_p)
{
...
translate_method_arg(char *method_arg, int *method_number)
{
    ...
    if (COMPARE(method_arg, "AIA_MCL_100US"))
    ...
    else if (COMPARE(method_arg, "BYTE_INTLV_MIDTOP_LINE"))
        *method_number = PDV_BYTE_INTLV_MIDTOP_LINE;
    else if (COMPARE(method_arg, "WORD_INTLV_MIDTOP_LINE"))
        *method_number = PDV_WORD_INTLV_MIDTOP_LINE;
    else if (COMPARE(method_arg, "WORD_INTLV_TOPMID_LINE"))
        *method_number = PDV_WORD_INTLV_TOPMID_LINE;
    else if (COMPARE(method_arg, "WORD_INTLV_HILO_LINE"))
        *method_number = PDV_WORD_INTLV_HILO_LINE;
    else if (COMPARE(method_arg, "WORD_INTLV_HILO"))
        *method_number = PDV_WORD_INTLV_HILO;
    else if (COMPARE(method_arg, "WORD_INTLV_TOPBOTTOM"))
        *method_number = PDV_WORD_INTLV_TOPBOTTOM;
    else if (COMPARE(method_arg, "BYTE_INTLV_INOUT"))
        *method_number = PDV_BYTE_INTLV_INOUT;
    else if (COMPARE(method_arg, "WORD_INTLV_INOUT"))
        *method_number = PDV_WORD_INTLV_INOUT;
    ...
    else if (COMPARE(method_arg, "DALSA_LS_4CH_INTLV"))
        *method_number = PDV_DALSA_LS_4CH_INTLV;
    ...
    else if (COMPARE(method_arg, "LINE_INTLV_P3_8X4"))
        *method_number = PDV_LINE_INTLV_P3_8X4;
    else if (COMPARE(method_arg, "INTLV_10BIT_8TAP_PACKED"))
    ...
    else if (COMPARE(method_arg, "TOSHIBA_SERIAL"))
        *method_number = TOSHIBA_SERIAL;
    else if (COMPARE(method_arg, "COHU_SERIAL"))
        *method_number = COHU_SERIAL;
    else if (COMPARE(method_arg, "ASCII_NOSPACE"))
        *method_number = SERIAL_ASCII_NOSPACE;
        ...
        *method_number = HeaderBefore;
    else if (COMPARE(method_arg, "HEADER_AFTER"))
        *method_number = HeaderAfter;
    else if (COMPARE(method_arg, "HEADER_WITHIN"))
        *method_number = HeaderBegin;
    else if (COMPARE(method_arg, "HEADER_BEGIN"))
        *method_number = HeaderBegin;
    else if (COMPARE(method_arg, "HEADER_MIDDLE"))
        *method_number = HeaderMiddle;
    else if (COMPARE(method_arg, "HEADER_END"))
        *method_number = HeaderEnd;
    ...
    else if (COMPARE(method_arg, "IRIG1"))
        *method_number = HDR_TYPE_IRIG1;
    else if (COMPARE(method_arg, "FRAMECNT"))
        *method_number = HDR_TYPE_FRAMECNT;
    else if (COMPARE(method_arg, "IRIG2"))
        *method_number = HDR_TYPE_IRIG2;
    else if (COMPARE(method_arg, "FRAMESYNC_OFF"))
        *method_number = PDV_FRAMESYNC_OFF;
    else if (COMPARE(method_arg, "FRAMESYNC_ON"))
        *method_number = PDV_FRAMESYNC_ON;
    else if (COMPARE(method_arg, "EMULATE_TIMEOUT"))
        *method_number = PDV_FRAMESYNC_EMULATE_TIMEOUT;
    else
        return 0;
...
set_method(Dependent * dd_p, Edtinfo * ei_p, char *method_type, int method_number)
{
    ...
    else if (is_method(method_type, "pixelclock_speed")) /* alias */
        dd_p->camera_data_rate = method_number;
    ...
    else if (is_method(method_type, "pingpong_varsize"))
        dd_p->pingpong_varsize = method_number;
    ...
    else if (is_method(method_type, "startdma"))
    {
        ...
        dd_p->startdma = method_number;
    }
    else if (is_method(method_type, "enddma"))
    {
        ei_p->enddma = method_number;
        dd_p->enddma = method_number;
    }

    else if (is_method(method_type, "flushdma"))
    {
        ei_p->flushdma = method_number;
        dd_p->flushdma = method_number;
    }

    else if (is_method(method_type, "header_position"))
        dd_p->header_position = method_number;

    else if (is_method(method_type, "header_type"))
        dd_p->header_type = method_number;

    else if (is_method(method_type, "framesync"))
        dd_p->framesync_mode = method_number;
    ...
    int
is_method(char *method_type, char *name)
{
    char    method_name[64];
 ...
 */
    void
clear_dependent(Dependent * dd_p)
{
 ...
 */
    int
check_int_method(char *line, char *label, int *arg, const char *cfgfile)
{
    char    format[32];
    int     ret = 0;
    int     n;
    ...
    int
check_ulong_method(char *line, char *label, u_long * arg, const char *cfgfile)
{
    char    format[32];
    int     ret = 0;
    int     n;
    ...
    int
check_float_method(char *line, char *label, float * arg, const char *cfgfile)
{
    char    format[32];
    int     ret = 0;
    int     n;
    ...
    int
check_ushort_method(char *line, char *label, u_short * arg, const char *cfgfile)
{
    char    format[32];
    int     ret = 0;
    int     n;
    ...
    int
check_byte_method(char *line, char *label, u_char * arg, const char *cfgfile)
{
    char    format[32];
    int     ret = 0;
    int     n;
 ...
 */
    int
check_str_method(char *line, char *label, char *arg, int max, const char *cfgfile)
{
    int     ret = 1;
    char   *p = NULL;
    char   *sp = NULL;
    ...
    int
check_path_method(char *line, char *label, char *arg, const char *cfgfile)
{
    int     ret = 0;
    int     n;
    char    format[32];
...
/* ALERT: check if this is same as check_foi_init -- hopefully so */
    int
check_serial_init_method(char *line, char *label, char *cmd, const char *cfgfile)
{
    char   *p = NULL;
    char   *sp = NULL;
    char   *ep;
 ...
 */
    int
check_hex_method(char *line, char *label, u_int * method, const char *cfgfile)
{
    char    format[32];
    char    hexstr[32];
 ...
 */
    int
check_other_param(char *s, Dependent * dd_p, Edtinfo *ei_p, const char *cfgfile)
{
    int     ret;
    int     tr;
    char    tmpserial[MAXINIT+2];
    ...
    if ((ret = check_int_method(s, "hwpad", &dd_p->hwpad, cfgfile)))
        return ret;
    if ((ret = check_int_method(s, "rgb30", &dd_p->rgb30, cfgfile)))
        return ret;
    ...
    if ((ret = check_int_method(s, "fv_done", &dd_p->fval_done, cfgfile))) /* alias */
        return ret;
    if ((ret = check_int_method(s, "pingpong_varsize", &dd_p->pingpong_varsize, cfgfile)))
        return ret;
    ...
    if ((ret = check_int_method(s, "shutter_speed_max", &dd_p->shutter_speed_max, cfgfile)))
        return ret;
    /* aliases */
    if ((ret = check_int_method(s, "exposure_min", &dd_p->shutter_speed_min, cfgfile)))
        return ret;
    if ((ret = check_int_method(s, "exposure_max", &dd_p->shutter_speed_max, cfgfile)))
        return ret;
    ...
    if ((ret = check_int_method(s, "fieldid_trig", &dd_p->fieldid_trig, cfgfile)))
        return ret;
    if ((ret = check_int_method(s, "slaved_trig", &tr, cfgfile)))
    {
        dd_p->photo_trig = 1;
        dd_p->fieldid_trig = 1;
        return ret;
    ...
    if ((ret = check_int_method(s, "sim_enable", &dd_p->sim_enable, cfgfile)))
        return ret;
    if ((ret = check_int_method(s, "startup_delay", &dd_p->startup_delay, cfgfile)))
        return ret;
    ...
    if ((ret = check_int_method(s, "mc4", &dd_p->mc4, cfgfile)))
        return ret;
    if ((ret = check_int_method(s, "expexsync", &dd_p->mc4, cfgfile)))
        return ret;
    ...
    if ((ret = check_int_method(s, "serial_init_delay", &ei_p->serial_init_delay, cfgfile)))
        return ret;

    if ((ret = check_int_method(s, "mode16", &dd_p->mode16, cfgfile)))
        return ret;

    if ((ret = check_byte_method(s, "irig_raw", &dd_p->irig_raw, cfgfile)))
        return ret;
    if ((ret = check_byte_method(s, "irig_offset", &dd_p->irig_offset, cfgfile)))
        return ret;
    if ((ret = check_byte_method(s, "irig_slave", &dd_p->irig_slave, cfgfile)))
        return ret;
    ...
    if ((ret = check_hex_method(s, "CL_CFG2_NORM", (u_int *) &dd_p->cl_cfg2, cfgfile)))
    {
        dd_p->cameralink = 1;
        return ret;
    }
    ...
    if ((ret = check_hex_method(s, "CL_MGTSPEED_NORM", (u_int *) &ei_p->cl_mgtspeed, cfgfile)))
    {
        dd_p->cameralink = 1;
        return ret;
    }

    if ((ret = check_hex_method(s, "trigdiv", (u_int *) &dd_p->trigdiv, cfgfile)))
        return ret;
    ...
    /* alias */
    if ((ret = check_hex_method(s, "serial_waitchar", &dd_p->serial_waitc, cfgfile)))
        return ret;
    ...
    if ((ret = check_str_method(s, "serial_exposure", dd_p->serial_exposure, MAXSER, cfgfile)))
        return ret;
    if ((ret = check_str_method(s, "serial_prefix", dd_p->serial_prefix, MAXSER, cfgfile)))
        return ret;
        ...
        return ret;
    {
        ...
        if ((ret = check_str_method(s, "line_interleave", line_interleave, 80, cfgfile)))
        {
            ...
            for (i=0;i<dd_p->n_intlv_taps;i++)
            {
                ...
                {
                    dd_p->intlv_taps[i].startx = atoi(tok1);
                    dd_p->intlv_taps[i].dx = atoi(tok2);
                }
            }
    ...
    int
check_cls_param(char *s, PdvDependent *dd_p, const char *cfgfile)

{
    ...
    {
        dd_p->cls.flags.Cfg.trigframe = val;
        return ret;
    }

    if ((ret = check_int_method(s, "cls_trigline", (int *) &val, cfgfile)))
    {
        dd_p->cls.flags.Cfg.trigline = val;
        return ret;
    }
    ...
    if ((ret = check_ushort_method(s, "cls_hgap", &dd_p->cls.hblank, cfgfile)))
        return ret;
    if ((ret = check_ushort_method(s, "cls_hcntmax",
                    &dd_p->cls.Hcntmax, cfgfile)))
        return ret;

    if ((ret = check_ulong_method(s, "cls_vgap", (u_long *)&dd_p->cls.vblank, cfgfile)))
        return ret;
    if ((ret = check_ulong_method(s, "cls_vcntmax", (u_long *)&dd_p->cls.Vcntmax, cfgfile)))
        return ret;

    if ((ret = check_ushort_method(s, "cls_hfvstart", &dd_p->cls.Hfvstart, cfgfile)))
        return ret;
    if ((ret = check_ushort_method(s, "cls_hfvend", &dd_p->cls.Hfvend, cfgfile)))
        return ret;

    if ((ret = check_ushort_method(s, "cls_hlvstart", &dd_p->cls.Hlvstart, cfgfile)))
        return ret;
    if ((ret = check_ushort_method(s, "cls_hlvend", &dd_p->cls.Hlvend, cfgfile)))
        return ret;

    if ((ret = check_ushort_method(s, "cls_hrvstart", &dd_p->cls.Hrvstart, cfgfile)))
        return ret;
    if ((ret = check_ushort_method(s, "cls_hrvend", &dd_p->cls.Hrvend, cfgfile)))
        return ret;
    ...
    int
check_xilinx_param(char *s, Dependent * dd_p, const char *cfgfile)
{
    int     index = 0;
    u_int   reg = 0;
    u_int   value = 0;
    ...
    int
check_method_param(char *s, Dependent * dd_p, Edtinfo * ei_p, const char *cfgfile)
{
    int     ret = 0;
    int     method_number;
...
printcfg(Dependent * dd_p)
{
    ...
    edt_msg(DEBUG2, "framesync_mode %d\n", dd_p->framesync_mode);
...
strip_extra_whitespace(char *s)
{
    ...
    int
resolve_cameratype(Dependent *dd_p)
{
    char tmpstr[256];
