 ...
 * libdvu.c
 *
 * EDT digital imaging device support utility defines and routines
 *
...
/**
 * Utility routine that outputs a 1-band, 8-bit image to a Sun raster format
 * file (regardless of platform).
 *
 * @param fname	    the name of the output file
 * @param addr      the address of the image data (8 bits per pixel)
 * @param x_size    width in pixels of image
 * @param y_size    height in pixels of image
 *
 * @return  
...
dvu_write_rasfile(char *fname, u_char *addr, int x_size, int y_size)
{
    ...
    if (Pdv_debug) printf("before write %d %d\n",x_size,y_size*3) ;
    for(i = 0; i < y_size ; i++)
    {
...
/**
 * converts 1 band, 10-16 bit image to a sun raster format file and
 * writes to a file.
 *
 * @param fname       the name of the output file
 * @param addr        the address of the image data (8 bits per pixel)
 * @param x_size      width in pixels of image
 * @param y_size      height in pixels of image
 * @param depth_bits  number of bits per pixel
 *
 * @return
 *	 0 on success, -1 on failure
 *
	...
	
{
	...
	if (Pdv_debug) printf("writing rasterfile header to %s\n", fname);
	if (Pdv_debug) printf("x %d y %d addr %p name %s\n", x_size, y_size, addr, fname);
	rowsize = x_size ;
	if(rowsize & 1)
	{
		rowsize++ ;
...
dvu_write_window(char *fname, dvu_window *w)
{
    ...
    {
	if (Pdv_debug)
	    printf("dvu_write_window does not yet support 24 bit images\n") ;
	return(-1) ;
    }
    if (bytes == 2)
    {
...
dvu_read_window(char *fname)
{
	...
	edt_msg(
		DVUFATAL,
		"%s not a sun raster file - exiting\n", fname);
	return(0) ;
    }
...
/**
 * Utility routine that outputs a 1-band, 8-bit image to a Sun raster format
 * file (regardless of platform), with stride. This function can be used to
 * output a partial image. For example, to output the top left 100x100 pixels of
 * the image, specify \a x_size of 100 and \a y_size * 100, and an \a istride of
 * the actual width of the image.
 *
 * @note Use #dvu_write_rasfile to output a full image in the most efficient way.
 *
 *
 * @param fname	    the name of the output file
 * @param addr	    the address of the image data (8 bits per pixel)
 * @param x_size    width in pixels of image
 * @param y_size    height in pixels of image
 * @param istride   number of pixels (bytes) to skip from one the beginning of one line
 * to the beginning of the next (this should just be the image width, unless you
 * want something weird like a diagonally skewed image).
 *
 * @return
 *	 0 on success, -1 on failure
 *
 ...
 * @param addr      data buffer, three bytes per pixel (RGB)
 * @param x_size    number of pixels per line
 * @param y_size    number of lines in the image
 *
 * @return 
 *	 0 on success, -1 on failure
 *
...
/**
 * Writes a 24-bit per pixel RGB data buffer as a Sun Raster format file, with
 * stride.
 *
 * @note To output a full image in the most efficient way, use #dvu_write_rasfile24.
 *
 * @param fname    the name of output file
 * @param addr     data buffer, three bytes per pixel (RGB)
 * @param x_size   number of pixels per line 
 * @param y_size   number of lines in the image
 * @param istride  number of pixels to skip between sucessive lines
 *
 * @Example
 * @code
...
int
dvu_wordscale(u_short *words, u_char *bytes, int count, 
			  int minbyte, int maxbyte, int doinit) 
{
    int i ;
    if (doinit)
    {
...
dvu_histeq(u_char *src, u_char *dst, int size, int depth)
{
    ...
    if(Pdv_debug) printf("dvu_histeq %p %p %d %d\n",src,dst,size,depth) ;
...
dvu_exp_histeq(u_char *src, u_char *dst, int size, int depth, int cutoff)
{
    ...
    int ncolors = 1 << depth ;
    int ret ;
    int min, max, distinct ;
    float inc ;
    float val ;
    int ival ;

    if(Pdv_debug) printf("dvu_exp_histeq %p %p %d %d\n",src,dst,size,depth) ;
	...
	if (Pdv_debug > 2) printf("%d (%d) %3.2f %d\n",
		i,hist[i],val,ival) ;
	if (depth > 8)
	    slookup[i] = ival ;
	else
 ...
 */
int
dvu_word2byte_with_stride(u_short *wbuf, u_char *bbuf, 
 ...
 * @param buffer   data buffer, one byte per pixel
 * @param width    number of pixels (bytes) per line
 * @param height   number of lines in the image
 *
 * @return 0 on success, -1 on failure. 
 * 
 * @see #dvu_write_bmp_24
 */
...
int
dvu_write_bmp_24(char *fname, u_char *buffer, int width, int height)
...
/**
 * Writes a 24-bit per pixel RGB data buffer as a raw data file (no
 * formatting).
 * 
 * @Example
 * @code
 ...
 */
void
dvu_long_to_charbuf(unsigned int val, u_char *buf)
{
