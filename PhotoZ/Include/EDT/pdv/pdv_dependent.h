...
typedef struct _PdvDependent
{
    ...
    char    serial_init[256];  /* moved to Edtinfo so no longer used EXCEPT in pdv_flshow; can repurpose when that goes away */
    char    foi_init[256];
    char    xilinx_init[256];
    ...
    int     frame_timing ;
    int     register_wrap ;
    char   *serial_binit ; /* moved to Edtinfo so no longer used EXCEPT in pdv_flshow; can repurpose when that goes away */
    int	   fval_done;
    int    cl_hmax;
    int    serial_init_delay ; /* moved to Edtinfo so no longer used EXCEPT in pdv_flshow; can repurpose when that goes away */
    int    mode16;
