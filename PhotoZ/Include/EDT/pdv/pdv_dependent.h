...
typedef struct _PdvDependent
{
  ...
  char serial_init[OLDMAXINIT]; /* moved to Edtinfo; not used EXCEPT pdv_flshow; can overload when
                                   that goes away */
  ...
  int frame_timing;
  int register_wrap;
  char *serial_binit; /* moved to Edtinfo; not used EXCEPT pdv_flshow; can overload when that goes
                         away */
  int fval_done;
  int cl_hmax;
  int serial_init_delay; /* moved to Edtinfo; not used EXCEPT pdv_flshow; can overload when that
                            goes away */
  int mode16;
...
#define PDV_QUADRANT3_INTLV \
  94 /* 4 port deintlv, like QUADRANT_INTLV but starting in the middle, iterating out */
